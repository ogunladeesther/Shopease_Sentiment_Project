...
def predict_user_sentiment(request: TextRequest):
    try:
        result = predictor.predict_sentiment(request.text)
        top_label = max(result, key=lambda x: x['score'])
        return {"label": top_label['label'], 
...
async def predict_batch(file: UploadFile = File(...)):
    ...
    # check if the required column 'review' is present
    ...
    for idx, row in df.iterrows():
        try:
            ...
            logging.error(f"Error for review {idx}: {e}\n{error_details}")
            result_row = row.to_dict()
