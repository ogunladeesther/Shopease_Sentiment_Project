...
class data_processor:
    def __init__(self):
        # Reading the dataset using the data_ingestion function from data_ingestion.py
        self.data = data_ingestion()
        # Cleaning the data using the clean data from data_cleaning.py
        self.clean_data = DataCleaning().clean_data(self.data)

    def split_data(self):
        try:
            # Features which is the preprocessed data
            x = self.clean_data['final_text'].astype(str)
            # The labels / Target of the dataset
...
class TokenizerWrapper:
    ...
    # FIX 1: indented encode() inside the class
...
class SentimentDataset(torch.utils.data.Dataset):
    def __init__(self, encodings, labels):
        ...
        # ensure labels are in list format to avoid any pandas index issue
...
def Prepare_sentiment_data():
    try:
        processsor = data_processor()
        X_train, X_test, y_train, y_test = processsor.split_data()
        # FIX 2: removed the early return
        tokenizer = TokenizerWrapper()
        # FIX 3: corrected .encoder() to .encode()
        ...
        logging.info(f"Data has been successfully prepared and ready to be passed to the model....") 
        return train_dataset, test_dataset
    except Exception as e:
        logging.error(f"Error occurred while preparing and processingthe data: {e}")
